...
$x=mysql_connect("localhost","root","") or die("echec de connectin au server ".mysql_error());
	...
	if (($res[0]!=$_POST['nom'])||($_POST['id']!=$res[1])) {
		echo "
<script language=javascript>alert('Vous n\'ete pas dans notre base de donnees faites vous enregistrer par l\' administrateur');</script><p style='font-size:48px;color:red'>Erreur de connection</p>
		";
	}else{
		$sx=mysql_query("select sexe from comptable where nom='$nom' and identifiant='$identifiant'");
		$sx=mysql_fetch_row($sx);
